#!/usr/bin/env python3
"""
Plot observed rates from traderpros run.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Plot observed rates from traderpros run.")
    ...
    # Extract columns
    extinction_cols = [col for col in df.columns if col.startswith('extinction_observed[')]
    pop_formation_cols = [col for col in df.columns if col.startswith('pop_formation_observed[')]
    spCompletion_cols = [col for col in df.columns if col.startswith('spCompletion_observed[')]
    transition_rates_cols = [col for col in df.columns if col.startswith('transition_rates[')]

    # Rename
    extinction_rename = {col: rename_observed(col, args.n_observed_states) for col in extinction_cols}
    pop_formation_rename = {col: rename_observed(col, args.n_observed_states) for col in pop_formation_cols}
    spCompletion_rename = {col: rename_observed(col, args.n_observed_states) for col in spCompletion_cols}
    ...
    # Save
    pdf_path = os.path.join(args.output, 'Traderpros_Rates_Obs_Plot.pdf')
    tiff_path = os.path.join(args.output, 'Traderpros_Rates_Obs_Plot.tiff')
