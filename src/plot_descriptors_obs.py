#!/usr/bin/env python3
"""
Plot observed descriptors from traderpros run.
"""
...
def rename_descriptor_obs(col, n_obs):
    idx = int(col.split('[')[1].rstrip(']')) - 1
    return str(idx)

def main():
    parser = argparse.ArgumentParser(description="Plot observed descriptors from traderpros run.")
    ...
    # Extract columns
    ephemerality_cols = [col for col in df.columns if col.startswith('ephemerality_observed[')]
    evolvability_cols = [col for col in df.columns if col.startswith('evolvability_observed[')]
    net_pop_formation_rate_cols = [col for col in df.columns if col.startswith('net_pop_formation_rate_observed[')]
    pop_turnover_cols = [col for col in df.columns if col.startswith('pop_turnover_observed[')]

    # Rename
    ephemerality_rename = {col: rename_descriptor_obs(col, args.n_observed_states) for col in ephemerality_cols}
    evolvability_rename = {col: rename_descriptor_obs(col, args.n_observed_states) for col in evolvability_cols}
    net_pop_formation_rate_rename = {col: rename_descriptor_obs(col, args.n_observed_states) for col in net_pop_formation_rate_cols}
    pop_turnover_rename = {col: rename_descriptor_obs(col, args.n_observed_states) for col in pop_turnover_cols}
    ...
    # Save
    pdf_path = os.path.join(args.output, 'Traderpros_Descriptors_Obs_Plot.pdf')
    tiff_path = os.path.join(args.output, 'Traderpros_Descriptors_Obs_Plot.tiff')
