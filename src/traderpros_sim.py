...
def create_revscript(args):
    ...
    for (n in 1:n_simulations){{
        ...
        distanceMatrix <- fnTreePairwiseDistances(SpeciationBranchTree[n])
        
        num_tips <- SpeciationBranchTree[n].ntips()
        groups <- rep(-1, num_tips)  # Initialize with -1 indicating unassigned
        group_count <- 1
        ...
        # Create an array of tip names and groups
        tipNames <- SpeciationBranchTree[n].taxa()
        ...
        for (i in 1:tipNames.size()) {{
            write(tipNames[i].getName(),
                  groups[i],
                  filename=outFolder + outPrefix + ".SpeciesMatrix.Sim" + n + ".txt",
                  append=TRUE)
            write("\\n",
        ...
        if(n_unknown_sp  > 0){{
          ...
          final_line <-  initial_line + n_unknown_sp
